...
- **Valores por defecto inseguros**: tanto `backend/db.js` como `backend/middleware/auth.js` tienen un *fallback* hardcodeado (`'TU_PASSWORD'` y `'clave_secreta_constructora'` respectivamente) si no se definen las variables de entorno correspondientes. Deben sobrescribirse siempre con `.env` antes de exponer el servicio, y nunca dejarse en un despliegue real.
